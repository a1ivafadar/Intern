...
class CallingViewModel: NSObject, ObservableObject {
    ...
    @Published var incomingCallPushNotification: PushNotificationInfo?
    ...
    func unRegisterPushNotifications() {
        self.callAgent?.unregisterPushNotification(completionHandler: { (error) in
            if (error != nil) {
    ...
    func handlePushNotification(incomingCallPushNotification: PushNotificationInfo) {
    ...
    func getCurrentCallUUID() -> UUID? {
        if let call = self.call,
           let callId = UUID(uuidString: call.id) {
            return callId
        } else {
            return nil
        }
    ...
    func getCall(callId: UUID) -> Call? {
        if let call = self.call {
            if (call.id == callId.uuidString.lowercased()) {
                return call
            } else {
                return nil
            }
            ...
            if let callAgent = self.callAgent {
                ...
                self.getDeviceManager { _ in
                    if let localVideoStream = self.localVideoStream {
                        ...
                        callAgent.join(with: groupCallLocator, joinCallOptions: joinCallOptions, completionHandler: { Call, Error in
                            self.call = Call
                        })
            ...
            if let callAgent = self.callAgent {
                ...
                self.getDeviceManager { _ in
                    if let localVideoStream = self.localVideoStream {
                        ...
                        //self.call = callAgent.calls(participants: callees, options: startCallOptions)
                        self.call?.delegate = self
                        let callId = UUID(uuidString: (self.call?.id)!)
                        CallController.shared.startCall(callId: callId!, handle: Constants.displayName, isVideo: true) { error in
                            if let error = error {
            ...
            self.requestRecordPermission { authorized in
                if authorized {
                    ...
                    self.getDeviceManager { _ in
                        ...
                        call.startVideo (stream: self.localVideoStream!, completionHandler: { error in
                            if let error = error {
...
//        if let addedCall = args.addedCalls.first(where: {$0. }) {
...
//        }
        ...
        if let call = self.call,
           let removedCall = removedCalls.first(where: {$0.id == call.id}),
...
extension CallingViewModel: CallDelegate {
    ...
    private func updateDisplayedRemoteParticipants() {
        for remoteParticipant in remoteParticipants {
            if displayedRemoteParticipants.count < CallingViewModel.remoteParticipantsDisplayed,
...
//
//        if call.state == .connected && !call.direction {
